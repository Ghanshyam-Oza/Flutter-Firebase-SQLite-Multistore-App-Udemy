...
import 'package:multistore_supplier/minor_screens/forgot_password.dart';
...
class _SupplierLoginScreenState extends State<SupplierLoginScreen> {
  ...
  Future<bool> validSupplier(String email) async {
    var matched = false;
    await FirebaseFirestore.instance
        .collection("suppliers")
        .get()
        .then((value) {
      for (final supplier in value.docs) {
  ...
  void onSubmit() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        var isValidSupplier = await validSupplier(email);
        if (isValidSupplier == true) {
          ...
          MySnackBar.showSnackBar(
              context: context, content: "Sorry, You are not valid Supplier.");
        }
