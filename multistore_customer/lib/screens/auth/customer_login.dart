...
class _CustomerLoginScreenState extends State<CustomerLoginScreen> {
  ...
  Future<bool> validCustomer(String email) async {
    var matched = false;
    await FirebaseFirestore.instance
        .collection("customers")
        .get()
        .then((value) {
      for (final customer in value.docs) {
  ...
  void onSubmit() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        var isValidCustomer = await validCustomer(email);
        if (isValidCustomer == true) {
          ...
          MySnackBar.showSnackBar(
              context: context, content: "Sorry, You are not valid customer.");
        }
